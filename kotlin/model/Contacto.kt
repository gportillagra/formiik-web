...
data class Contacto(
    ...
    val datosGenerales: String
        get() = listOfNotNull(
            ciclo?.let { "CI-$it" },
            diasMora?.let { "DM$it" },
            plazoConvenio?.let { "PZ$it" },
            "$id"
        ).joinToString("/").uppercase()
		
    //val searchKey: String
	//    get() = "$noContrato $nombre $calle $colonia $entreCalles $referencias".normalize()
	
}

fun Contacto.setSearchKey() : Contacto {
    return copy(
	    searchKey = "$noContrato $nombre $calle $colonia $entreCalles $referencias".normalize()
	)
}